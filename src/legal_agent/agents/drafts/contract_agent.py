...
=== EMPLOYMENT CLAUSE CHECKLIST (when sub_type = "employment") ===
Every employment draft MUST include these sections, in this order, each as a `##` heading. This checklist is binding — a draft missing any of these is incomplete. Where a template reference is supplied, follow its numbering; where none is supplied, use the numbering below.

1. Title + execution recital (`THIS EMPLOYMENT AGREEMENT ... made and executed at [City] on this [today's date] ... BETWEEN: [Employer] ... AND [Employee]`)
2. `## RECITALS` — WHEREAS clauses + "NOW, THEREFORE" articulation
3. `## 1. APPOINTMENT AND POSITION` — role, reporting, duties
4. `## 2. COMMENCEMENT, PROBATION AND TERM` — start date (default = execution date if silent), probation (default 6 months), term
5. `## 3. PLACE OF WORK AND WORKING HOURS` — office, remote-work policy, hours
6. `## 4. REMUNERATION AND CTC` — CTC figure + reference to Schedule A for the breakup; TDS under §192 Income Tax Act; annual review; variable pay
7. `## 5. STATUTORY BENEFITS` — Provident Fund (EPF Act 1952, 12% each way of Basic); ESI (ESI Act 1948, subject to wage ceiling); Gratuity (Payment of Gratuity Act 1972, 15 days' Basic per year after 5 years); Group Medical Insurance; Group Term Life Insurance; Professional Tax
8. `## 6. LEAVE POLICY` — reference to Schedule B + accrual/carry-forward/encashment rules + Maternity Benefit Act 1961 (as amended 2017) for 26 weeks
9. `## 7. POSH ACT COMPLIANCE` — Internal Committee under §4 PoSH Act 2013; reference to employer's PoSH Policy under §19; training/sensitisation obligation
10. `## 8. CONFIDENTIALITY` — definition of Confidential Information; obligations; exceptions (public, prior, independent, legal compulsion); return on termination; survival
11. `## 9. INTELLECTUAL PROPERTY` — assignment of work product; moral-rights waiver; pre-existing IP disclosure + carve-out; further-assurances clause
12. `## 10. NON-SOLICITATION AND NON-COMPETE` — MUST contain 10.1 Non-Solicitation of Employees (12 months post-term), 10.2 Non-Solicitation of Clients (12 months post-term), 10.3 Non-Compete (narrow, 12 months, respecting §27 Contract Act), 10.4 Reasonableness & severability of restraints. If you only draft a non-compete without non-solicit, rename the heading to `## 10. NON-COMPETE` — do NOT keep the compound heading with only one part.
13. `## 11. DATA PROTECTION` — DPDP Act 2023 compliance (purpose limitation, consent, data-principal rights, breach notification within 24 hours to the Data Protection Board). Do NOT cite IT Act §43A / SPDI Rules.
14. `## 12. TERMINATION` — 12.1 Termination for Convenience (90 days post-confirmation); 12.2 Termination for Cause (misconduct, breach, fraud, absenteeism ≥ 7 days, conviction for moral turpitude, PoSH harassment, material breach); 12.3 Resignation (90 days notice or salary in lieu); 12.4 Garden Leave
15. `## 13. RETURN OF PROPERTY` — laptop, mobile, ID card, access cards, documents, credentials, Confidential Information copies; certification of compliance as condition for full-and-final settlement
16. `## 14. INDEMNIFICATION` — mutual (employee indemnifies for wilful misconduct / breach / unauthorised acts; employer indemnifies for acts within authorised scope)
17. `## 15. GOVERNING LAW AND DISPUTE RESOLUTION` — Indian law; amicable resolution (30 days); arbitration under A&C Act 1996, sole arbitrator, seat = Employer's registered-office city; interim relief from competent courts at the seat city
18. `## 16. SURVIVAL` — explicitly list which clauses survive termination (confidentiality, IP, non-solicit, non-compete, data protection, return of property, indemnification, governing law)
19. `## 17. MISCELLANEOUS` — MUST include 17.1 Notices, 17.2 Entire Agreement, 17.3 Amendment, 17.4 Severability, 17.5 Waiver, 17.6 Assignment (restricted for Employee, permitted for Employer in M&A), 17.7 Counterparts and Electronic Execution (§5 IT Act), 17.8 Stamp Duty (per applicable State Stamp Act, burden assignment)
20. **IN WITNESS WHEREOF** execution block — pipe table with two columns (Employer / Employee), each row: Signature line, Name, Designation (Employer only), Date (DD/MM/YYYY), Place
21. **WITNESSES** pipe table — two columns (Witness 1 / Witness 2), each row: Name, Address, Signature line
22. `## SCHEDULE A — SALARY BREAKUP` — populated pipe table with Basic (50% of gross), HRA (40% of Basic), Special Allowance, Leave Travel Allowance, Employer PF Contribution (12% of Basic), Gratuity provision (4.81% of Basic), Group Medical premium, Group Term Life premium, Total Fixed CTC; variable-pay note; statutory-deduction note (employee PF, Professional Tax, TDS §192)
23. `## SCHEDULE B — LEAVE POLICY` — populated pipe table with Casual Leave, Sick Leave, Earned/Privilege Leave, Public Holidays, Maternity Leave, Paternity Leave, Bereavement Leave + accrual/carry-forward note
24. `## SCHEDULE C — JOB DESCRIPTION` — populated with key responsibilities + qualifications
...
4. **Digital Personal Data Protection Act, 2023 (DPDP Act)** — the data-protection clause in every contract involving personal-data processing MUST cite the DPDP Act, 2023. Cover: purpose limitation, consent, data-principal rights, breach notification to the Data Protection Board, data-fiduciary obligations. Do NOT cite IT Act §43A or the SPDI Rules in a data-protection clause — they are legacy provisions superseded by DPDP for employment and most commercial personal-data processing. Mandatory in employment agreements (employee PII) and in service / technology agreements that touch customer data.
...
7. **Information Technology Act, 2000** — cite ONLY for electronic execution validity: §10A (validity of electronic contracts) and §5 (validity of electronic signatures), subject to the Second Schedule exclusions. Do NOT cite §43A or the SPDI Rules for data-protection purposes — DPDP Act, 2023 governs that (see item 4 above).
