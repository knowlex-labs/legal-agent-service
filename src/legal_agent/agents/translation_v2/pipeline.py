...
the JobManager can tag the failure correctly.
"""
...
async def translate_pdf_v2(
    ...
    job_id: str,
    debug_dir: str | None = None,  # noqa: ARG001 — kept for signature parity with v1
) -> tuple[bytes, dict[str, Any]]:
    """Run the v2 translation pipeline. Returns (pdf_bytes, metadata_dict).

    Note: v2 deliberately does not emit per-stage debug dumps. `debug_dir` is
    accepted (ignored) only so the caller can share a single dispatch signature
    with v1. Inspect outputs via job metadata + the final PDF.
    """
    settings = get_settings()
    vision_model = settings.translation_v2_vision_model
    translate_model = settings.translation_v2_translate_model
    dpi = settings.translation_v2_target_dpi
    meta: dict[str, Any] = {
        "translation_pipeline": "v2",
        "extraction_route": "v2_gemini_html",
        "translation_v2_vision_model": vision_model,
        "translation_v2_translate_model": translate_model,
    ...
    t0 = time.perf_counter()
    glossary = await build_glossary(client, vision_pages, model=translate_model, job_id=job_id)
    ...
    logger.info(
        "[%s] v2 pipeline complete: %d pages, %d bytes "
        "(raster %d / vision %d / glossary %d / translate %d / html %d / pdf %d / concat %d ms)",
        job_id,
        len(per_page_pdfs),
        len(final_pdf),
        meta["rasterize_ms"],
